...
use PF\Slim\AdminRouteMiddleware;
use PF\Slim\Slim;
...
class Venue {
  public static function register() {
    ...
    $adminRouteMiddleware = new AdminRouteMiddleware();
    ...
    $app->group('/venue', function () use ($app, $entityManager, $serializer, $adminRouteMiddleware) {
      $app->get('/search', function () use ($app, $entityManager) {
        $venuesIterator = $entityManager->getRepository('\PF\Venue')
          ->getVenues($app->request());
      ...
      $app->post('/:id/approve', array(
        $adminRouteMiddleware,
        'call'
      ), function ($id) use ($app, $entityManager) {
      ...
      $app->post('', function () use ($app, $entityManager, $serializer) {
        ...
        if (!empty($json_venue_decoded['machines'])) {
          foreach ($json_venue_decoded['machines'] as $json_machine_decoded) {
            ...
            $game = $entityManager->getRepository('\PF\Game')
              ->find($json_machine_decoded['ipdb']);
